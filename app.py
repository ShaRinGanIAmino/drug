...
import numpy as np
from chembl_webresource_client.new_client import new_client  # CheMBL client :contentReference[oaicite:3]{index=3}
from rdkit import Chem
from rdkit.Chem import Lipinski                             # Lipinski filters :contentReference[oaicite:4]{index=4}
import streamlit as st                                       # Streamlit UI :contentReference[oaicite:5]{index=5}
...
def fetch_smiles(target_id='CHEMBL4497824', limit=1000):
    ...
    return [s for s in smiles if Chem.MolFromSmiles(s)]      # validity check :contentReference[oaicite:6]{index=6}
...
class VAE(nn.Module):
    def __init__(self, vocab_size, emb=64, hid=128, lat=64):
        ...
        self.mu = nn.Linear(hid, lat); self.logv = nn.Linear(hid, lat)
...
def generate(model, itos, n=5, maxlen=100):
    ...
    # start with all-pad (decoder ignores input char)
...
def lipinski_pass(smi):
    ...
    return sum([
        Lipinski.NHOHCount(m)>5, Lipinski.NOCount(m)>10,
        Chem.Crippen.MolLogP(m)>5, Chem.Descriptors.MolWt(m)>500
    ]) <= 1
